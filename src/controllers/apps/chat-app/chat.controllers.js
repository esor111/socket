...
console.log(new mongoose.Types.ObjectId(receiverId))
console.log({mab:req.user._id.toString()})
  // if not we need to create a new one on one chat
  const newChatInstance = await Chat.create({
    name: "One on one chat",
    participants: [req.user._id.toString(), new mongoose.Types.ObjectId(receiverId)], // add receiver and logged in user as participants
    admin: req.user._id,
  });
